...
def create_brca_data(downloaded_zip_file, test_subjects=None):

    # Check if the directory exists
    check_directory_exists(downloaded_zip_file)
    
    # extract zip file for images
    ...
    # Check if the directory exists
    check_directory_exists(subtracted_image_path)
    ...
    df.to_csv("brca_data.csv", index=False)
...
def read_config():
    ...
    Returns the path to the zip file containing the mammogram images.
    """
    # Create an argument parser
    parser = argparse.ArgumentParser(description="Process configuration parameters.")
    
    # Add argument for the zip file
    parser.add_argument(
        "--zip_file_path",
        help= "Path to the zip file containing mammogram images. Download from: https://www.cancerimagingarchive.net/collection/cdd-cesm/",
        # required=True
        default="CDD-CESM.zip"
    )
    
    # Add argument for the output_folder
    parser.add_argument(
        "--output_folder",
        help= "output folder to save .docx and .pdf files",
        default="create_image_and_annotation_folder"
    )
